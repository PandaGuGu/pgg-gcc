...
- **P4 拆分（2026-08-25 起草，待 B0 决策点确认）**：P4 分两期——一期 **v4 = 编译器核心子集
  （B1 生蛋原料，对标 subC 最小面：int/char、指针、数组、字符串、逻辑、控制流补充、全局、强转、
  注释）**，细则见 §4.4；二期 = P4 余项（struct/union/enum/typedef/const·volatile/位运算/三目/
  逗号/switch/sizeof/多维数组等）按 §5 继续排期，达成 C90 全量。
...
| P4 | 类型系统补全（**一期 v4=编译器核心子集 B1**：char/指针/数组/字符串/逻辑/do-while·break·continue/复合赋值/全局/强转/注释 → 自举原料；二期=struct/union/enum/typedef/const·volatile/位运算/三目/逗号/switch/sizeof → C90 全量） | C90 | ✅ v4（B1）2026-08-25 完成（72/72）；**P4-II 二期大部分 2026-08-26 已在自举链落地（run_boot 212/212）**——落地清单与残余限制见 ORIGIN P4-II 行、architecture-b2 §8/§8.1、日志会话 4–6；细则 §4.4（pggcc4 链） |
...
### 4.4 P4 一期 / v4（无蛋 stage-4 · 编译器核心子集 B1——生蛋原料）

> **定位**：从 P4（§5）裁剪"够写编译器本体"的最小面（对标 subC 能力面：int/char、数组、
> 指针、字符串、逻辑、控制流补充、全局变量），**箭头直插自举里程碑 B1→B2**（§6.3）。
> **v4 完成 = B1 达成**（生蛋原料齐备），随后即 B0 决策点（v4 后、P5 前）：确认 B2 用 pggcc
> 语言重写 B0 编译器源码。其余 C90 特性（struct/union/enum/typedef/const·volatile/位运算/
> 三目/逗号/switch/sizeof/多维数组等）为 **P4 二期**，v4 不吸收。
> 本细则按 §4.0–4.3 同风格给出：输入协议 / 词法 token 集 / 文法 EBNF / 语义 / 代码生成 / 自证要点 /
> 不包含面。公共约束同前：i386 AT&T 汇编、无 libc/crt、栈式求值；可推翻但须日志留痕。

**输入/输出协议**（v3 扩展）：stdin 程序文本；函数序列；入口固定 `main`；结束打印 `main` 返回值。
生成程序**首次引入数据段**：`.rodata`（字符串/常量）、`.data`（全局有初值）、`.bss`（全局零值）；
生成程序运行时助手**新增 `print_str`**（NUL 结尾字符串→stdout，自举链打印诊断文本用，随模板输出）。

**0. 类型系统（本阶段核心新增）**：符号表 `{name, off}` 升级为 `{name, off, tbase, tptr, anum}`：
- `tbase ∈ {T_INT, T_CHAR}`；`tptr = 指针层级（0 标量，1 一级指针，2 二级指针，上限 2）`；
  类型即 `(tbase, tptr)`：`int`=(T_INT,0) `char`=(T_CHAR,0) `int *`=(T_INT,1) `char **`=(T_CHAR,2)…
- 数组不单独成类型：`int a[N]` 登记 `(tbase, tptr=1, anum=N)`；`anum=0` 表示非数组；
  数组名作表达式 = 首元素地址（退化指针，只读，不可整体赋值）
- **char 定无符号（零扩展）**；char 变量占一个 4B 栈槽（低字节有效）、char 数组/全局元素为 1B
- 全局符号表独立 `{name, tbase, tptr, anum, init}`，跨函数可见（v4 起，函数表外新增全局表）

**1. 词法新增（token 集扩展）**：
- 关键字：`char`、`void`（仅函数返回类型）、`do`、`break`、`continue`（switch 不入面→无 case/default）
- 字符字面量 `'c'`：单字节；转义 `\n \t \\ \' \" \r \0`（token 值 = 字节 int）
- 字符串字面量 `"..."`：转义同上；入常量表（≤64 条、每条 ≤256B，表满 → 错误）；token 值 = 表序号；
  发码时进 `.rodata`
- 运算符：`[` `]`、`++` `--`、`&`（仅取址；位与属二期）、`!`、`&&`、`||`、`+=` `-=` `*=` `/=`
  （`*` 复用 TOK_STAR：声明/乘法/解引用按上下文区分，见文法）
- 强转 `(int)` `(char)` `(int *)` `(char *)`：见 primary 判定（`(` 后瞻 type 关键字 + `*`/`)`）
- 注释 `/* ... */`（C89 单层，不嵌套）：词法遇 `/` 后一字符 `*` 则跳过至 `*/`

**2. 文法**（EBNF；只列新增/修改，未列者同 v3）：
```
decl      := type dec (',' dec)* ';'
type      := 'int' | 'char' | 'void'          # void 仅函数返回
dec       := ptr IDENT array?                 # 数组仅一维；暂不支持复合声明符
ptr       := '*'*
array     := '[' NUM ']'
func      := type IDENT '(' params ')' block  # 返回 int/char/void；void 内可无 return
stmt      := 同 v3，另加：
           | 'do' stmt 'while' '(' expr ')' ';'
           | 'break' ';' | 'continue' ';'     # 仅循环内；break 亦可用于 do-while
assign    := lval ((OPASS)|ASN) assign | cmp  # OPASS ∈ += -= *= /=；赋值右结合
logic     := cmp (('&&'|'||') cmp)*           # 短路，结果 0/1；置于 cmp 之上
unary     := ('-'|'!'|'*'|'&') unary | postfix
postfix   := primary ('[' expr ']')* ('++'|'--')*
           | primary ('++'|'--')              # 连续下标后置（简化：下标在前、自增自减在后）
primary   := NUM | CHAR | STR | IDENT (call?) | '(' expr ')' | '(' type ')' unary
lval      := IDENT | '*' unary | postfix '[' expr ']'（求左值地址表达式）
```
> `arg/赋值右值/条件/下标 []` 一律经 parse_assign 入口（同 v3 机制）；
> `(` 强转判定：token `(` 后瞻——若为 `int`/`char` 且其后为 `*`/`)` 组合者据声明语法识别为类型转换，
> 否则按括号表达式；用 peek 双层前瞻消歧。

**3. 语义要点**：
- 指针算术：`p+i` → `p + i×sizeof(*p)`（`*p` 为 int/ptr 时 ×4，char 时 ×1）；`i+p` 同；`p-i` 同理
- 下标：`a[i] ≡ *(a+i)`；`&a[i]` = 地址表达式；数组名取值 = 首地址
- 解引用：读 `movzbl/movl (addr)`；写字节/字——读写目标扩展为**左值**（变量 | `*e` | `e[i]` 之后）
- 赋值/复合赋值/`++`/`--` 要求左值；复合赋值与前后缀：单次求左值地址，读旧值→运算→写回；
  前缀返回新值、后缀返回旧值（后缀实现=临时存旧值）
- 逻辑：`&&` 左假不求右、`||` 左真不求右（**短路**）；`!` 0/1；结果 int 0/1
- 强转：int↔char（截断/零扩展）；int↔指针（值重解释）；`int *`↔`char *`（字节用意自担）
- 全局：可初始化常量表达式（int N / char 'c' / `{1,2,...}` 列表 / 字符串 → `.data`；未初始化 → `.bss` 零）；
  全局符号跨函数可见，函数内可读写（`g` 直接寻址）
- char 打印：数值打印沿用 print_decimal；字符串打印用新增 print_str

**4. 代码生成**：
- 读：int/ptr `movl -off(%ebp),%eax`；char `movzbl -off(%ebp),%eax`（零扩展）；
  数组名 `leal 基址,%eax`（局部数组基址 = 栈中连续 N 元素区）
- 写：int/ptr `movl %eax,off`；char `movb %al,off`
- 元素/指针访问：`addr = base + idx×esz` 后 `[addr]` 读/写（esz：int/ptr=4，char=1）
  - 基址来源：数组名地址 / 指针值；局部数组基址 = 首元素栈地址（声明时 `subl $4N,%esp` 一次性/逐元素分配，记录首尾）
- **左值求址发码（约定）**：变量 → `leal -off(%ebp),%eax`；`*e` → e 值即地址；`e[i]` → 基址 + idx×esz；
  写回序列统一 `pushl 地址; <求右值→压栈>; popl %eax(右值); popl %ecx(地址); movx %eax,(%ecx); movzbl? 终值回栈`（链式赋值/表达式值语义）
- 全局：`movl g_sym` / `movb g_sym`（.data/.bss 符号）；取址 `leal g_sym,%eax`
- 字符串：`.rodata` `Lstr%d:` 输出（转义还原为字节，NUL 结尾）；使用处 = 地址入栈
- 逻辑短路：`&&` → `testl; jz Lf; <右>; testl; jz Lf; pushl $1; jmp Le; Lf: pushl $0; Le:`；
  `||` 对称（左真跳 Lt push 1）；`!` → `popl %eax; testl; sete %al; movzbl; pushl`
- 复合赋值右结合链：`a += b += c` 由 parse_assign 递归自然得到
- do-while：`LD: body; cond; testl; jnz LD`
- break/continue：维护**循环控制标号栈**（每开 do/while/for 压 `[cont, brk]` 两标号，块/函数结束校验落空）；
  break → `jmp Lbrk`；continue → `jmp Lcont`（for 的跳 inc、while/do 的跳 cond）
- 强转：int→char `movzbl %al,%eax`；char→int 已零扩展（无操作）；int↔ptr 无操作（4B 直传）
- 函数：void 无返回值（`ret` 前不取值）；char 返回低字节、调用方 movzbl 扩展

**5. 自证要点**（期望值手算；run.sh 成功用例 + 错误用例）：
1. 字符串：`int len(char *s){...}` 手写，`len("hello")==5`；字符数组 `char buf[8]` 逐字节写读
2. 数组：`sum(a,5)==15`；`a[2]==2`；`a` 与 `&a[0]` 相等（比较 ==1）
3. 指针：swap 交换两 int；指针遍历累加；`p++` 步进
4. char：`'A'+1=='B'`；char→int 零扩展（`'\xff'` 等显式值）
5. 逻辑短路：(1||f()) 不调用 f（副作用计数判 0）；`&&`、`!` 返回 0/1
6. do-while / break / continue：计数用例（continue 跳过累加、break 提前退出）
7. 复合赋值与 `++`/`--`：`s+=i`、`i++` 前后值（后缀返回旧值）
8. 全局变量跨函数累计；全局数组初始化和
9. 强转 `(char)i` 截断、`(int)c`、指针强转做字节读写
10. 综合：手写 `strcpy`（复制返回指针）、`strcmp`（==0/±），双函数互相调用——B2 的预演
- 错误用例：非左值赋值（`3=4`、`(a+b)=5`）、下标越界不查（登记限制）、数组整体赋值、
  越级指针运算（`p+i` 对 char* 的 ×1 步进）、break 在循环外 → exit 2

**6. 不包含（P4 二期，按 §5 继续排期）**：struct/union/enum/typedef/const·volatile/
位运算 `&|^~<<>>`/三目 `?:`/逗号表达式/switch·case·default/sizeof、多维数组、复合声明符
任意组合（`int *a[3]`）、字符串字面量拼接、unsigned/float/double、预处理器、变长函数、
K&R 旧式函数定义。**中间结果与符号地址 int32 域内，溢出/越界不查（登记限制）**。

**7. B1 达成判定（v4 验收门）**：
1. tests/run.sh 全部自证用例 PASS，构建链全程无 C/C++ 编译器；
2. 生成 `.s` 人工抽检：.rodata/.data/.bss 三段、左值求址/写回序列、逻辑短路标号、块栈回收沿用 v3；
3. ORIGIN v4 行登记（§6.1 符字面量/§6.3 增删改表达式·指针与地址/§6.5 声明/§6.6 语句/§6.7 全局）；
4. 未越红线；随后触发 **B0 决策点**（用户确认 B2 自举重写方案后再开工）。

## 5. P4–P8 核心特性集（明确开工面；标准驱动，逐版本吸收，每条在 ORIGIN 登记）

> 注：P4 已拆两期（见 §0、§3）——**一期 = v4 编译器核心子集（§4.4，已含优先级与裁剪面）**；
> 以下 P4 列表为全量清单，一期优先取 §4.4，其余为二期按序吸收。
...
| **B0 决策点（建议）** | 从 P4 裁出"够写编译器"的最小面（对标 subC：int/char、数组、指针、字符串、控制流、函数），箭头直插自举里程碑 B0。**推荐**（v4 细则见 §4.4） |
| P8 全 C89 后 | 特性面大、本体大，自举调试成本随规模上升；且晚拿"自举=自测 oracle"这一最大收益。不推荐 |

### 6.3 自举里程碑（B0–B6）

- **B0**：v3 控制流 + 递归自证（fact(5)==120）→ 语言图灵完备 = **硬门槛达成**。
- **B1**：v4 编译器核心子集（数组/指针/字符串/比较/逻辑/char；细则 §4.4，2026-08-25 起草）→ 生蛋原料。
