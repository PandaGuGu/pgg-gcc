...
| 阶段 | 目标 | 对应版本 | 状态 / 细则 |
|------|------|----------|--------------|
| P0（v0） | 词法 + 整数四则表达式 → 汇编（自研 i386 汇编 stage-0） | 起步 C89 子集 | ⏳ 待重推（无蛋），**细则 §4.0** |
| P1（v1） | 变量声明与赋值（符号表、栈帧局部变量） | C89 | 📋 **细则 §4.1** |
| P2（v2） | 函数定义与调用（栈帧、参数传递、返回） | C89 | 📋 **细则 §4.2** |
| P3（v3） | 控制流（if/while/for、块作用域、比较） | C89 | 📋 **细则 §4.3** |
