...
| 移植 P1 | ✅ **P1 完成**（2026-08-26，链上） | 轻量 IR v0.1（D1b 决策，platform-portability §5.1）：boot0 发码改走 ostr/oint → IR 缓冲 → irgo 统一渲染；**P1-b** em_* 全族语义化为操作码+参数（OP_PUSHI/PUSHL/STORE/BINOP/CMP/JZ/JNZ/JMP/LBL），irgo 按操作码分发（AMD64 渲染器选择点就位）；**run_boot 232/232、boot_b4 闭台全门 PASS、run.sh 72/72**；下一步 P2 AMD64 后端 |
