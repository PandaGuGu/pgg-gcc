...
| 移植 P2 | ✅ **P2-a 完成**（2026-08-26，链上）；📋 P2-b 待续 | AMD64 渲染链路（platform §5.2）：g_am 双目标（0=i386/1=amd64）；操作码 64 位化 + eh() 运行时模板 syscall 化 + 调用/栈帧 64 位；**tests/run_amd.sh 9/9**（bin_amd 直出 amd64.s → as --64 → ld → 本机原生运行：常量/算术/调用/递归/print_int/print_str/局部/负数）；x86 链零变化；下一步 P2-b 透传文本语义化 → P3 双目标闭台 |
