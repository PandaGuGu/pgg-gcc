...
| 移植 P1 | ✅ **P1-a + P1-b 首轮完成**（2026-08-26，链上）；📋 P1-b 续轮待续 | 轻量 IR v0.1 值流化（D1b 决策，platform-portability §5.1）：boot0 发码改走 ostr/oint → IR 缓冲 → irgo 统一渲染；**P1-b 首轮**：IR 类型扩展 2=操作码，em_p/em_ld/es_/em_jz/em_jnz/em_jmp/em_lbl 七结构化指令改操作码+参数、irgo 按操作码分发（AMD64 渲染器选择点就位）；**run_boot 232/232、boot_b4 闭台全门 PASS、run.sh 72/72**；下一步 P1-b 续轮（em_o/em_cm 语义化）→ P2 AMD64 后端 |
